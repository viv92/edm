...
class DPCT(nn.Module):
    ...
    def forward(self, x, # x.shape: [b, x_seq_len, x_dim] 
        ...
        dpct_out_seq = self.decoder(dpct_input_emb, encoder_out=condition_emb, src_mask_padding=None, tgt_mask_padding=None, tgt_mask_causal=None) # dpct_out_seq.shape: [batch_size, max_seq_len_dpct, d_model]
...
def init_dpct(max_seq_len_dpct, x_seq_len, d_model, x_dim, condition_dim, d_k, d_v, n_heads, n_layers, d_ff, dropout, device):
    assert max_seq_len_dpct == x_seq_len + 1
